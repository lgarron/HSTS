...
\begin{abstract}
HSTS (HTTP Strict Transport Security) has gained significant browser and server adoption since reaching IETF proposed status. However, there are several important deployment challenges. A scan of top websites reveals that many HSTS sites have not properly configured the HSTS header, which still leaves them open to some attacks HSTS is meant to solve.  We survey current deployment status and describe common mistakes and difficulties with HSTS configuration.  We conclude with an approach for properly deploying HSTS as effectively as possible.
\end{abstract}
