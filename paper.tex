...
$30\%$ of HSTS sites in our survey also send an HSTS header over \h or \hw. While an RFC-conforming browser will ignore this, RFC 6797\cite{rfc} also states that:
...
The strongest protection is to redirect every resource to HTTPS immediately and and serve an HSTS header:

\begin{itemize}
\item with every request,
\item with a long \code{max-age},
\item and with \iSD.
\end{itemize}

To be fully correct, the site must also never send an HSTS header over plain HTTP.
...
Any security-conscious site should strive to deploy this approach, if possible.

\subsection{Dealing with \iSD~Limitations}

\subsection{Securing \s~from \sw}

\subsection{Securing Redirects}

\section{Browser Considerations}

\subsection{Preload List}
